...
$('#loginForm').addEventListener('submit', async (e) => {
  ...
  const fb = $('.feedback', f); fb.textContent = ''; fb.className = 'feedback';
  try {
...
$('#registerForm').addEventListener('submit', async (e) => {
  ...
  const body = {
    ...
    company: f.company.value, cnpj: f.cnpj.value, phone: f.phone.value,
  };
  try {
...
async function loadProposals() {
  ...
  `).join('') || '<tr><td colspan="7" class="muted">Você ainda não submeteu nenhuma proposta.</td></tr>';
}
...
document.addEventListener('click', async (e) => {
  ...
  $('#detailContent').innerHTML = `
    ...
    <dl class="detail-grid">
      <dt>Proponente</dt><dd>${escapeHtml(p.proponente)}</dd>
      ...
      ${p.admin_notes ? `<dt>Retorno da equipe</dt><dd>${escapeHtml(p.admin_notes)}</dd>` : ''}
      <dt>Submetida</dt><dd>${fmtDate(p.created_at)}</dd>
      <dt>Atualizada</dt><dd>${fmtDate(p.updated_at)}</dd>
    </dl>
...
$('#proposalForm').addEventListener('submit', async (e) => {
  ...
  const fb = $('.feedback', f); fb.textContent = ''; fb.className = 'feedback';
  const body = {
    nome: f.nome.value, email: f.email.value,
    proponente: f.proponente.value, cnpj: f.cnpj.value,
    perfil: f.perfil.value, area: f.area.value, estagio: f.estagio.value,
    objetivo: f.objetivo.value, resumo: f.resumo.value,
  };
  try {
    await api('/me/proposals', { method: 'POST', body: JSON.stringify(body) });
    fb.textContent = 'Proposta enviada! Acompanhe o status em "Minhas propostas".';
    fb.classList.add('ok');
    f.reset();
    prefillProposalForm();
  ...
  try {
    await api('/me', {
      ...
      body: JSON.stringify({
        ...
        cnpj: f.cnpj.value, phone: f.phone.value,
      }),
    });
    ...
    $('.user-name').textContent = state.user.name;
    fb.textContent = 'Dados atualizados.';
    fb.classList.add('ok');
  } catch (err) { fb.textContent = err.message; fb.classList.add('error'); }
});
